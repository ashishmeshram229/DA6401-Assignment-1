...
def parse_arguments():
    # Parse CLI arguments for training script and return args object.
...
def main():
    # Parse CLI arguments and normalise certain arguments (activation, loss name, hidden_size list) for consistency and safety
    args = parse_arguments()



    if isinstance(args.activation, list): # Normalise activation to always be a single string (shouldn't happen if config is correct, but added for safety)
    ...
    if len(args.hidden_size) < args.num_layers:
        ...
        args.hidden_size = args.hidden_size[:args.num_layers] # Trim hidden_size list to match num_layers if it's too long (shouldn't happen if config is correct, but added for safety)
