...
def load_model(model_path, config_path=None):
    ...
    class A: pass
    a = A()
    a.dataset       = cfg.get("dataset",       "fashion_mnist")
    a.loss          = cfg.get("loss",          "cross_entropy")
    a.optimizer     = cfg.get("optimizer",     "rmsprop")
    a.learning_rate = cfg.get("learning_rate", 0.001)
    a.weight_decay  = cfg.get("weight_decay",  0.0)
    a.num_layers    = cfg.get("num_layers",    3)
    a.hidden_size   = cfg.get("hidden_size",   [128, 128, 128])
    a.activation    = cfg.get("activation",    ["relu", "relu", "relu"])
    a.weight_init   = cfg.get("weight_init",   "xavier")
    a.wandb_project = cfg.get("wandb_project", "da6401_assignment_1")

    n = max(1, a.num_layers)
    if not isinstance(a.hidden_size, list): a.hidden_size = [a.hidden_size] * n
    if not isinstance(a.activation,  list): a.activation  = [a.activation]  * n
    a.hidden_size = (a.hidden_size + [a.hidden_size[-1]] * n)[:n]
    a.activation  = (a.activation  + [a.activation[-1]]  * n)[:n]

    model = NeuralNetwork(a)

    raw = np.load(model_path, allow_pickle=True)
    model.load_weights(raw.item() if (isinstance(raw, np.ndarray) and raw.ndim == 0) else raw)
    return model, a
